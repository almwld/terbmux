    ...
    public static Error deleteFile(String label, final String filePath, final boolean ignoreNonExistentFile, final boolean ignoreWrongFileType, int allowedFileTypeFlags) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                try { java.nio.file.Files.walk(file.toPath()).sorted(java.util.Comparator.reverseOrder()).map(java.nio.file.Path::toFile).forEach(java.io.File::delete); } catch (java.io.IOException e) { /* ignore */ }
            } else {
                if (fileType == FileType.DIRECTORY) {
        ...
        try {
            ...
            if (fileType == FileType.DIRECTORY) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    ...
                    //noinspection UnstableApiUsage
                    java.io.File[] files = file.listFiles(); if (files != null) { for (java.io.File f : files) { try { java.nio.file.Files.walk(f.toPath()).sorted(java.util.Comparator.reverseOrder()).map(java.nio.file.Path::toFile).forEach(java.io.File::delete); } catch (java.io.IOException e) { /* ignore */ } } }
                } else {
                    // Will give runtime exceptions on android < 8 due to missing classes like java.nio.file.Path if org.apache.commons.io version > 2.5
